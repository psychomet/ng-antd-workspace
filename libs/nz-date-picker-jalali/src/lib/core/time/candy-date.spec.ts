...
export const candyDateFac = (date?: Date | number | string): CandyDate =>
  new CandyDate(dateAdapter, date);

describe('candy-date coverage supplements', () => {
  const date = candyDateFac('2018-5-5 12:12:12');

  it('support getTime', () =>
    expect(date.getTime()).toBe(date.nativeDate.getTime()));
  ...
  it('support isSame', () => {
    ...
    expect(
      date.isSame(candyDateFac('2018-5-5 12:12:00'), 'minute')
    ).toBeTruthy();
    expect(date.isSameMinute(candyDateFac('2019-5-5 12:12:00'))).toBeFalsy();

    expect(
      date.isSame(candyDateFac('2018-5-5 12:12:12'), 'second')
    ).toBeTruthy();
  ...
  it('should normalizeRangeValue work', () => {
    ...
    result = normalizeRangeValue(
      dateAdapter,
      [randomDay, candyDateFac()],
      false,
      'month',
      'left'
    );
    ...
    expect(
      differenceInCalendarMonths(result[1]!.nativeDate, randomDay.nativeDate)
    ).toEqual(1);
