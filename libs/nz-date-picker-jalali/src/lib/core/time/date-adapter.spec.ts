import { dateAdapter } from '../core/time/candy-date.spec';
...
describe('date-adapter', () => {
  ...
  it('should support calendarStartOfWeek with different weekStartsOn', function () {
    ...
    expect(dateAdapter.calendarStartOfWeek(date, { weekStartsOn: 1 })).toEqual(
      startOfWeek
    );
  });
  ...
  it('should support isBefore', () => {
    ...
    expect(
      dateAdapter.isBefore(date, new Date('2018-06-12'), 'year')
    ).toBeFalse();
    expect(
      dateAdapter.isBefore(date, new Date('2020-07-12'), 'month')
    ).toBeTrue();
    expect(
      dateAdapter.isBefore(date, new Date('2020-06-13'), 'day')
    ).toBeTrue();
  });
