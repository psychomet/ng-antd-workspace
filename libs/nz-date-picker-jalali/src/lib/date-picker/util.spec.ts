import { CandyDate, DateFnsDateAdapter } from '../core/time';

import { isAllowedDate } from './util';

const dateAdapter = new DateFnsDateAdapter();
const candyDateFac = (date?: Date | number | string): CandyDate =>
...
describe('util.ts coverage supplements', () => {
  it('should cover untouched branches', () => {
    ...
    expect(
      isAllowedDate(
        candyDateFac('2000-11-11 01:11:11'),
        undefined,
        disabledTime
      )
    ).toBeFalsy();
    expect(
      isAllowedDate(
        candyDateFac('2000-11-11 02:02:11'),
        undefined,
        disabledTime
      )
    ).toBeFalsy();
    expect(
      isAllowedDate(
        candyDateFac('2000-11-11 02:03:03'),
        undefined,
        disabledTime
      )
    ).toBeFalsy();
